...
def get_output_path(model_repr:str, args) -> str:
    ...
    path = f"{args.custom_output_dir}/{model_repr}/{scenario}_{n}_{temperature}{cot_suffix}.json"
    ensure_dir(path)
    return path
...
def get_eval_all_output_path(model_repr:str, args) -> str:
    ...
    temperature = args.temperature
    cot_suffix = "_cot" if args.cot_code_execution else ""
    path = f"{args.custom_output_dir}/{model_repr}/{scenario}_{n}_{temperature}{cot_suffix}_eval_all.json"
    ensure_dir(path)
    return path
